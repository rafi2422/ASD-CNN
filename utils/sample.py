...
def display_pred(output_dir, pred, file_names, labels, subject, model_size,classes, kaggle):
    ...
    for i in range (0,trials):
        ...
        if p_class !=labels[i]: 
            errors=errors + 1
            e_list.append(file_names[i]) 
            true_class.append(classes[labels[i]]) 
            pred_class.append(classes[p_class])
            prob_list.append(100 *pred[i][p_class])
...
def TF2_classify(source_dir, output_dir, mode, subject, v_split=5, epochs=20, batch_size=80,
    ...
    while run:
        ...
        for d in vloss_new:
            vloss.append(d)
        last_epoch=results.epoch[len(results.epoch)-1] 
        tr_plot(tacc,vacc,tloss,vloss)
        bestw=val.best_weights 
        lr_rate=val.lr
